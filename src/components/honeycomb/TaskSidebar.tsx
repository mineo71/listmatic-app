...
const TaskSidebar = ({
  ...
  const groupedItems = useMemo(() => {
    if (groupBy === 'none') {
      return { 'All Tasks': filteredAndSortedItems };
    }
    ...
    return filteredAndSortedItems.reduce((groups: Record<string, HoneycombItem[]>, item) => {
      ...
      switch (groupBy) {
        case 'color':
          groupKey = item.color || 'No Color';
          break;
        case 'completion':
          groupKey = item.completed ? 'Completed' : 'Pending';
          break;
        case 'deadline':
          if (!item.deadline) {
        ...
        {showFilters && (
          <div className="pt-2 space-y-2">
            ...
            >
              ...
              <option value="completion">Group by Status</option>
